document.querySelector('.container').addEventListener('click', pek);
function pek(event) {
    if (event.target.classList.contains('chikibriki')) {
        window.location.href = 'https://www.youtube.com/watch?v=BnTW6fZz-1E';
    }
}

document.querySelector('.container2').addEventListener('mouseleave', hover);
function hover() {
    document.querySelector('.container2').classList.toggle('j');
}

document.addEventListener('scroll', () => {
    const clone = document.querySelector('.container2 div').cloneNode();
    clone.classList.add('n');
    document.body.insertAdjacentElement('beforeend', clone);
});

document.addEventListener('mousemove', (event) => {
    if (event.target.classList && event.target.classList.contains('n')) {
        event.target.remove();
    }
});
